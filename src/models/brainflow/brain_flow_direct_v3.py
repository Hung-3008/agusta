...
class BrainFlowDirectV3(nn.Module):
    ...
    def __init__(
        ...
        self.reg_output = SubjectLayers(hidden_dim, output_dim, n_subjects, bias=True)
    ...
    def forward(
        ...
        if subject_ids is not None:
            fmri_pred = self.reg_output(reg_hidden, subject_ids)
        else:
            avg_w = self.reg_output.weights.mean(dim=0)
            fmri_pred = reg_hidden @ avg_w
            if self.reg_output.bias is not None:
                fmri_pred = fmri_pred + self.reg_output.bias.mean(dim=0)
