...
from src.datasets.bmd_dataset import get_bmd_dataloaders
...
def train(args):
    ...
    train_loader, val_loader = get_bmd_dataloaders(cfg)
    _dl = cfg["dataloader"]
    ...
    output_dim = bf_cfg.get("output_dim", train_loader.dataset.n_voxels)
    ...
    for epoch in range(start_epoch, tr_cfg["n_epochs"] + 1):
        ...
        if epoch % tr_cfg["val_every_n_epochs"] == 0 or args.fast_dev_run:
                ...
                for batch_idx, batch in enumerate(tqdm(val_loader, desc="Val")):
                    ...
                    for b in range(B):
                        ...
                        if ck not in fmri_pred_acc:
                            fmri_pred_acc[ck] = {"sum": torch.zeros(fmri_dim), "count": 0}
                            fmri_tgt_acc[ck] = {"sum": torch.zeros(fmri_dim), "count": 0}

                        fmri_pred_acc[ck]["sum"] += gen_fmri[b].cpu()
                        fmri_pred_acc[ck]["count"] += 1
                        fmri_tgt_acc[ck]["sum"] += fmri_target[b].cpu()
                        fmri_tgt_acc[ck]["count"] += 1
            ...
            for ck in sorted(fmri_pred_acc.keys()):
                ...
                if count < 1:
                    continue
                all_gen.append(fmri_pred_acc[ck]["sum"] / count)
                all_tgt.append(fmri_tgt_acc[ck]["sum"] / fmri_tgt_acc[ck]["count"])

            if all_gen:
                all_gen = torch.stack(all_gen, dim=0)
                all_tgt = torch.stack(all_tgt, dim=0)
