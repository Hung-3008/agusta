"""Train BrainFlow Direct V2 — End-to-End Flow Matching with Raw Multimodal Features.

No DAE bottleneck. Loads raw pre-pooled features per modality from NPY files.

Usage:
    python src/train_brainflow_direct_v2.py --config src/configs/brain_flow_direct_v2.yaml --fast_dev_run
    python src/train_brainflow_direct_v2.py --config src/configs/brain_flow_direct_v2.yaml
    python src/train_brainflow_direct_v2.py --config src/configs/brain_flow_direct_v2.yaml --resume
"""
...
logger = logging.getLogger("train_brainflow_direct_v2")
...
# =============================================================================
# Dataset — Multilayer Features + fMRI (LRU Cached)
# =============================================================================

class DirectFlowDatasetV2(Dataset):
    """Loads multimodal features + fMRI. Uses LRU Cache to avoid OOM."""
    ...
    def __init__(self, cfg, split="train"):
        ...
        self.modalities_cfg = cfg["modalities"]
        self.modality_names = sorted(self.modalities_cfg.keys())

        self.fmri_dir = Path(cfg["_fmri_dir"])
        self.features_dir = Path(PROJECT_ROOT) / cfg["features_dir"]
        ...
        if self.use_global_stats:
            for subj in self.subjects:
                ...
                if mean_path.exists() and std_path.exists():
                    self.fmri_mean[subj] = np.load(mean_path).astype(np.float32)
                    self.fmri_std[subj] = np.load(std_path).astype(np.float32)
                else:
                    logger.warning("Global stats not found for %s", subj)

        self.max_cache_gb = cfg["dataloader"].get("max_cache_gb", 40.0)
        self.cache = ClipLRUCache(max_gb=self.max_cache_gb)
    ...
    def _load_fmri_clip(self, subject, task, clip_name):
        ...
        if self.use_global_stats and subject in self.fmri_mean:
            data = (data - self.fmri_mean[subject][None, :]) / self.fmri_std[subject][None, :]
        return data
    ...
    def _build_index(self):
        ...
        for subj in self.subjects:
            ...
            for task, splits in self.splits_cfg.items():
                ...
                for stim_type in stim_types:
                    ...
                    first_subdir = self.modalities_cfg[first_mod]["subdir"]
                    clip_dir = self.features_dir / first_subdir / task / stim_type
                    ...
                    for clip_stem in clip_stems:
                        ...
                        # Validate fMRI exists without loading full data initially
                        fmri_path = _get_fmri_filepath(str(self.fmri_dir), subj, task)
                        if not fmri_path.exists():
                            logger.warning(f"Warning: {fmri_path} not found.")
                            continue
                        with h5py.File(fmri_path, "r") as f:
                            fmri_key = norm_name
                            if task == "friends" and norm_name.startswith("friends_"):
                                fmri_key = norm_name[len("friends_"):]
                            elif task == "movie10" and norm_name.startswith("movie10_"):
                                fmri_key = norm_name[len("movie10_"):]
                            matched = [k for k in f.keys() if fmri_key in k]
                            if not matched:
                                matched = [k for k in f.keys() if clip_stem in k]
                            if not matched:
                                continue
                            n_trs = len(f[matched[0]])
                            n_trs = n_trs - self.excl_start - self.excl_end if self.excl_end > 0 else n_trs - self.excl_start

                        # Collect feature paths
                        mod_paths = {}
                        for mod_name in self.modality_names:
                            subdir = self.modalities_cfg[mod_name]["subdir"]
                            feat_path = self.features_dir / subdir / task / stim_type / f"{clip_stem}.npy"
                            if not feat_path.exists():
                                feat_path = self.features_dir / subdir / task / stim_type / f"{norm_name}.npy"
                            if feat_path.exists():
                                mod_paths[mod_name] = feat_path

                        if not mod_paths:
                            continue

                        clip_id = f"{subj}/{task}/{stim_type}/{norm_name}"
                        self._clip_paths[clip_id] = {
                            "subject": subj,
                            "task": task,
                            "norm_name": norm_name,
                            "clip_stem": clip_stem,
        ...
        logger.info("Found %d clips → %d samples for %s split.", n_clips, len(samples), self.split)
        return samples

    def _get_loaded_clip(self, clip_id):
        cached = self.cache.get(clip_id)
        if cached is not None:
            return cached

        paths = self._clip_paths[clip_id]
        subj = paths["subject"]
        task = paths["task"]
        norm_name = paths["norm_name"]
        clip_stem = paths["clip_stem"]
        ...
        mod_features = {}
        total_bytes = fmri_data.nbytes if fmri_data is not None else 0
        for mod, path in paths["mod_paths"].items():
            feat_data = np.load(path).astype(np.float32)
            feat_data = np.nan_to_num(feat_data, nan=0.0, posinf=0.0, neginf=0.0)
            mod_features[mod] = feat_data
            total_bytes += feat_data.nbytes

        clip_data = {"fmri": fmri_data, "features": mod_features}
        self.cache.put(clip_id, clip_data, total_bytes)
        return clip_data
    ...
    def __getitem__(self, idx):
        ...
        n_trs = info["n_trs"]
        subj_idx = info["subject_idx"]

        clip = self._get_loaded_clip(clip_id)
        fmri_data = clip["fmri"]
        mod_features = clip["features"]
        ...
        result_features = {}
        for mod_name in self.modality_names:
            mod_dim = self.modalities_cfg[mod_name]["dim"]
            expected_layers = self.modalities_cfg[mod_name].get("layers", 1)
            if mod_name in mod_features:
                feat_data = mod_features[mod_name]
                # feat_data shape can be (T, dim) or (T, L, dim)
                safe_start = max(0, feat_start)
                safe_end = min(feat_data.shape[0], feat_end)
                
                if safe_start < safe_end:
                    feat_window = feat_data[safe_start:safe_end].copy()
                else:
                    feat_window = np.zeros((0, *feat_data.shape[1:]), dtype=np.float32)
                
                if feat_window.shape[0] < self.feat_seq_len:
                    pad_len = self.feat_seq_len - feat_window.shape[0]
                    pad_width = [(pad_len, 0)] + [(0,0)] * (feat_window.ndim - 1)
                    feat_window = np.pad(feat_window, pad_width, mode="constant")
                
                # Normalize shape: ensure (T, L, D) if layers > 1
                if expected_layers > 1 and feat_window.ndim == 2:
                    feat_window = np.expand_dims(feat_window, axis=1)  # (T,D) -> (T,1,D)
                    feat_window = np.repeat(feat_window, expected_layers, axis=1)  # broadcast
            else:
                if expected_layers > 1:
                    feat_window = np.zeros((self.feat_seq_len, expected_layers, mod_dim), dtype=np.float32)
                else:
                    feat_window = np.zeros((self.feat_seq_len, mod_dim), dtype=np.float32)
            result_features[mod_name] = torch.from_numpy(feat_window)
...
def collate_fn(batch):
    ...
    clip_keys = [b["clip_key"] for b in batch]
    target_trs = torch.tensor([b["target_tr"] for b in batch])
    n_trs = torch.tensor([b["n_trs"] for b in batch])
    subject_idx = torch.tensor([b["subject_idx"] for b in batch])

    mod_names = sorted(batch[0]["features"].keys())
    features = {}
    for mod in mod_names:
        features[mod] = torch.stack([b["features"][mod] for b in batch])

    return {
        "features": features,
        "fmri": fmri,
        "clip_key": clip_keys,
        "target_tr": target_trs,
        "n_trs": n_trs,
...
def get_dataloaders(cfg):
    train_set = DirectFlowDatasetV2(cfg, split="train")
    val_set = DirectFlowDatasetV2(cfg, split="val")
...
def train(args):
    ...
    modality_dims = {
        name: mc["dim"] for name, mc in cfg["modalities"].items()
    }
    ...
    # 4. Output dir
    out_dir = Path(PROJECT_ROOT) / cfg.get("output_dir", "outputs/brainflow_direct_v2")
    ...
    for epoch in range(start_epoch, tr_cfg["n_epochs"] + 1):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            with torch.amp.autocast("cuda", enabled=tr_cfg["use_amp"], dtype=torch.bfloat16):
                loss = model(features, fmri_target, subject_idx=subject_idx)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="src/configs/brain_flow_direct_v2.yaml")
